...
info_rectangle_size = 40
size_of_square = screen_resolution / amount_of_rows
number_of_mines = 60

# INITIALIZE GAME
pygame.init()
screen = pygame.display.set_mode(size=(screen_resolution, screen_resolution + info_rectangle_size),
                                 flags=pygame.SCALED)
...
def get_index_from_square(square):
  ...
  for row in squares:
    col_number = 0
    for col in row:
      if (col["rectangle"].top
          == square["rectangle"].top) and (col["rectangle"].left
...
def remove_empty_squares(pos, exempt_squares=()):
  ...
  for cord in neighbor_cords:
    if (0 <= cord[0]) and (cord[0] <= amount_of_rows - 1):
      if (0 <= cord[1]) and (cord[1] <= amount_of_rows - 1):
        if squares[cord[0]][cord[1]] in exempt_squares:
          continue
        if squares[cord[0]][cord[1]]["is_cleared"] == True:
          continue
